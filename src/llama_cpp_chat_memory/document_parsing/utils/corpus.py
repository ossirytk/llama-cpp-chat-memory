...
from document_parsing import extract, spacier
from document_parsing import io as tio
from document_parsing.utils import errors, types, utils
...
class Corpus:
    ...
    def add(self, data: types.CorpusData, batch_size: int = 1000, n_process: int = 1):
        ...
        if isinstance(data, str):
            ...
            if isinstance(first, str):
                ...
                raise TypeError(errors.type_invalid_msg("data", type(data), types.CorpusData))
        else:
            raise TypeError(errors.type_invalid_msg("data", type(data), types.CorpusData))
    ...
    def remove(
        ...
        """
        matched_docs = (doc for doc in self.docs if match_func(doc) is True)
        self._remove_many_docs_by_index(self._doc_ids.index(id(doc)) for doc in itertools.islice(matched_docs, limit))
    ...
    def word_counts(
        self,
        *,
        by: Literal["lemma", "lower", "norm", "orth", "lemma_", "lower_", "norm_", "orth_"] = "lemma",
        ...
        **kwargs,
        ...
        if weighting == "count":
            ...
            word_counts_ = {word: count / n_tokens for word, count in word_counts_.items()}
        else:
            raise ValueError(errors.value_invalid_msg("weighting", weighting, {"count", "freq"}))
        return word_counts_
    ...
    def word_doc_counts(
        ...
        **kwargs,
        ...
        if weighting == "count":
            ...
            n_docs = self.n_docs
            word_doc_counts_ = {word: count / n_docs for word, count in word_doc_counts_.items()}
        elif weighting == "idf":
            n_docs = self.n_docs
            if smooth_idf is True:
                word_doc_counts_ = {word: math.log1p(n_docs / count) for word, count in word_doc_counts_.items()}
            else:
                word_doc_counts_ = {word: math.log(n_docs / count) for word, count in word_doc_counts_.items()}
        else:
            raise ValueError(errors.value_invalid_msg("weighting", weighting, {"count", "freq", "idf"}))
