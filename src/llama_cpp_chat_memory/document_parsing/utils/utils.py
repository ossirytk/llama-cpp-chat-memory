...
from document_parsing.utils import errors as errors_
...
def deprecated(
    message: str,
    *,
    action: Literal["default", "error", "ignore", "always", "module", "once"] = "always",
):
    """
...
def is_record(obj: Any) -> bool:
    ...
    if isinstance(obj, tuple) and len(obj) == 2 and isinstance(obj[0], str) and isinstance(obj[1], dict):
        return True
    else:
        return False
...
def get_kwargs_for_func(func: Callable, kwargs: dict[str, Any]) -> dict[str, Any]:
    ...
    func_params = {name for name, param in inspect.signature(func).parameters.items() if param.kind in _KW_PARAM_KINDS}
    func_kwargs = {kwarg: value for kwarg, value in kwargs.items() if kwarg in func_params}
    return func_kwargs
