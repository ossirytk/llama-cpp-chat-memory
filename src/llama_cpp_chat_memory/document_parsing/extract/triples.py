...
from document_parsing.utils import constants, types, utils
...
DQTriple: tuple[list[Token], list[Token], Span] = collections.namedtuple("DQTriple", ["speaker", "cue", "content"])
...
def subject_verb_object_triples(doclike: types.DocLike) -> Iterable[SVOTriple]:
    ...
    for sent in sents:
        ...
        verb_sos: Mapping = collections.defaultdict(lambda: collections.defaultdict(set))
        for tok in sent:
            head = tok.head
        ...
        for verb, so_dict in verb_sos.items():
            ...
            if not so_dict.get("objects"):
                so_dict["objects"].update(
                    obj for conj in conjuncts for obj in verb_sos.get(conj, {}).get("objects", [])
                )
...
def semistructured_statements(
    ...
    *,
    ...
    if fragment_len_range is not None:
        fragment_len_range = utils.validate_and_clip_range(fragment_len_range, (1, 1000), int)
            ...
            if cue_cand.pos in {VERB, AUX} and cue_cand.lemma_ == cue:
                ...
                for tok in cue_cand.children:
                    if (
                        ...
                        or tok.dep_ == "dative"
                        or (tok.dep == xcomp and not any(child.dep == dobj for child in cue_cand.children))
                    ):
                        subtoks = list(tok.subtree)
                        if fragment_len_range is None or fragment_len_range[0] <= len(subtoks) < fragment_len_range[1]:
                            frag_cand = subtoks
                            break
    ...
    for qtok_start_idx, qtok_end_idx in qtok_pair_idxs:
        ...
        cue_cands = [
            ...
            if (
                ...
                and not any(qts_idx <= tok.i <= qte_idx for qts_idx, qte_idx in qtok_pair_idxs)
            )
        ]
