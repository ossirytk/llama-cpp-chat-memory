...
from document_parsing import similarity
    ...
    *,
    ...
    if edge_weighting == "count":
        cooc_counts = collections.Counter(
            w1_w2 for window in windows for w1_w2 in itertools.combinations(sorted(window), 2)
        )
        graph.add_edges_from((w1, w2, {"weight": weight}) for (w1, w2), weight in cooc_counts.items())
    elif edge_weighting == "binary":
        edge_data = {"weight": 1}
        graph.add_edges_from(
            (w1, w2, edge_data) for window in windows for (w1, w2) in itertools.combinations(window, 2)
        )
    else:
        raise ValueError(errors.value_invalid_msg("edge_weighting", edge_weighting, {"count", "binary"}))
        ...
        for vertex in l_step_expanded_set:
            ...
            for w in s1:
                try:
                    ...
                    LOGGER.error("Word %s not in contrib dict! We're approximating...", w)
            taken[vertex] = True
