...
from document_parsing.utils import errors
...
class Vectorizer:
    ...
    def _validate_vocabulary(self, vocabulary: dict[str, int] | Iterable[str]) -> tuple[dict[str, int], bool]:
        """
    ...
    def id_to_term(self) -> dict[int, str]:
        ...
        if len(self.id_to_term_) != self.vocabulary_terms:
            self.id_to_term_ = {term_id: term_str for term_str, term_id in self.vocabulary_terms.items()}
    ...
    def terms_list(self) -> list[str]:
        ...
        return [term_str for term_str, _ in sorted(self.vocabulary_terms.items(), key=operator.itemgetter(1))]
    ...
    def _fit(self, tokenized_docs: Iterable[Iterable[str]]) -> sp.csr_matrix:
        ...
        doc_term_matrix, vocabulary_terms = self._count_terms(tokenized_docs, self._fixed_terms)

        if self._fixed_terms is False:
            # filter terms by doc freq or info content, as specified in init
            doc_term_matrix, vocabulary_terms = self._filter_terms(doc_term_matrix, vocabulary_terms)
            # sort features alphabetically (vocabulary_terms modified in-place)
            doc_term_matrix = self._sort_vocab_and_matrix(doc_term_matrix, vocabulary_terms, axis="columns")
        ...
        if self.idf_type:
            ...
            self._idf_diag = sp.spdiags(idfs, diags=0, m=n_terms, n=n_terms, format="csr")
        ...
        if self.tf_type == "bm25" and self.dl_type:
            ...
            self._avg_doc_length = get_doc_lengths(doc_term_matrix, type_=self.dl_type).mean()
    ...
    def _reweight_values(self, doc_term_matrix: sp.csr_matrix) -> sp.csr_matrix:
        ...
        if self.tf_type == "binary":
            ...
            if not self.dl_type:
                doc_term_matrix.data = doc_term_matrix.data * (BM25_K1 + 1.0) / (BM25_K1 + doc_term_matrix.data)
            else:
            ...
            raise ValueError(
                errors.value_invalid_msg("tf_type", self.tf_type, {"binary", "bm25", "sqrt", "log", "linear"})
            )
        ...
        if self.norm is not None:
            doc_term_matrix = normalize_mat(doc_term_matrix, norm=self.norm, axis=1, copy=False)
...
class GroupVectorizer(Vectorizer):
    ...
    def id_to_grp(self) -> dict[int, str]:
        ...
        if len(self.id_to_grp_) != self.vocabulary_grps:
            self.id_to_grp_ = {grp_id: grp_str for grp_str, grp_id in self.vocabulary_grps.items()}
    ...
    def grps_list(self) -> list[str]:
        ...
        """
        self._check_vocabulary()
        return [grp_str for grp_str, _ in sorted(self.vocabulary_grps.items(), key=operator.itemgetter(1))]

    def fit(self, tokenized_docs: Iterable[Iterable[str]], grps: Iterable[str]) -> GroupVectorizer:
        """
        Count terms in ``tokenized_docs`` and, if not already provided, build up
    ...
    def fit_transform(self, tokenized_docs: Iterable[Iterable[str]], grps: Iterable[str]) -> sp.csr_matrix:
        """
        Count terms in ``tokenized_docs`` and, if not already provided, build up
        a vocabulary based those terms; do the same for the groups in ``grps``.
        Fit and store global weights (IDFs) and, if needed for term weighting,
    ...
    def transform(self, tokenized_docs: Iterable[Iterable[str]], grps: Iterable[str]) -> sp.csr_matrix:
        """
    ...
    def _fit(self, tokenized_docs: Iterable[Iterable[str]], grps: Iterable[str]) -> sp.csr_matrix:
        """
        Count terms and, if :attr:`Vectorizer.fixed_terms` is False, build up
        a vocabulary based on the terms found in ``tokenized_docs``. Transform
        ``tokenized_docs`` into a document-term matrix with absolute tf weights.
        ...
        if self._fixed_terms is False:
            # filter terms by doc freq or info content, as specified in init
            grp_term_matrix, vocabulary_terms = self._filter_terms(grp_term_matrix, vocabulary_terms)
            # sort features alphabetically (vocabulary_terms modified in-place)
            grp_term_matrix = self._sort_vocab_and_matrix(grp_term_matrix, vocabulary_terms, axis="columns")
            # *now* vocabulary_terms are known and fixed
            self.vocabulary_terms = vocabulary_terms
            self._fixed_terms = True
        if self._fixed_grps is False:
            # sort groups alphabetically (vocabulary_grps modified in-place)
            grp_term_matrix = self._sort_vocab_and_matrix(grp_term_matrix, vocabulary_grps, axis="rows")
        ...
        if self.tf_type == "bm25" and self.dl_type:
            # store the avg document length, used in bm25 weighting to normalize
            # term weights by the length of the containing documents
            self._avg_doc_length = get_doc_lengths(grp_term_matrix, type_=self.dl_type).mean()
