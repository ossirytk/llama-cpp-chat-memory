...
from document_parsing.utils import cache, constants, errors, types, utils
...
def make_doc_from_text_chunks(text: str, lang: types.LangLike, chunk_size: int = 100000) -> Doc:
    """
...
def get_main_verbs_of_sent(sent: Span) -> list[Token]:
    ...
    return [tok for tok in sent if tok.pos == VERB and tok.dep_ not in constants.AUX_DEPS]
...
def get_span_for_verb_auxiliaries(verb: Token) -> tuple[int, int]:
    ...
    """
    min_i = verb.i - sum(
        1 for _ in itertools.takewhile(lambda x: x.dep_ in constants.AUX_DEPS, reversed(list(verb.lefts)))
    )
    max_i = verb.i + sum(1 for _ in itertools.takewhile(lambda x: x.dep_ in constants.AUX_DEPS, verb.rights))
    return (min_i, max_i)
